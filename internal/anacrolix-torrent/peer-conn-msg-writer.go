...
func (cn *peerConnMsgWriter) write(msg pp.Message) bool {
	cn.mu.Lock()
	defer cn.mu.Unlock()
	// V239-Optimization: Zero-Allocation Write
	startPos := cn.writeBuffer.Len()
	// Write dummy length (4 bytes)
	cn.writeBuffer.Write([]byte{0, 0, 0, 0})

	err := msg.MarshalTo(cn.writeBuffer)
	if err != nil {
		panic(err)
	}

	endPos := cn.writeBuffer.Len()
	length := uint32(endPos - startPos - 4)

	// Backfill length
	binary.BigEndian.PutUint32(cn.writeBuffer.Bytes()[startPos:], length)
